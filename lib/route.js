...
Router.route('/',{
   ...
   if (Meteor.userId() ) {
      ...
      if(userDoc) {
         if(userDoc.quarterInfo[0].hasLent && userDoc.quarterInfo[0].hasBorrowed) {
             Router.go("userHome");
         }else{
            Router.go('suggestedCourses');
         }
      }else{
             Router.go('suggestedCourses');
      }
...
/*************************************************/
Router.route('/suggestedCourses', {
   name: "suggestedCourses",
   template: "suggestedCourses",
    waitOn: function() {return [
      Meteor.subscribe('departments'),
      Meteor.subscribe('courses'),
      Meteor.subscribe('textbooks'),
